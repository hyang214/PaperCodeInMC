...
public class JudgeFrequency {
	public Attribute Judgement(Map<String,List<Attribute>> allGap,Attribute base) {
		...
		Map<Integer,List<Integer>> candidateGap = new HashMap<Integer,List<Integer>>();
		...
		Set<Attribute> orderSet = new TreeSet<Attribute>();
		Iterator<Attribute> iterator = gapList.iterator();
		while (iterator.hasNext()) {
			Attribute gap = iterator.next();				
			orderSet.add(gap);									
		}
	
		getCandidateGapSet(candidateGap,orderSet);
		...
		while(index != null && index.size() != 0) {
			...
			if(selectedGap.upperBound - selectedGap.lowerBound + 1 <= 5) {
