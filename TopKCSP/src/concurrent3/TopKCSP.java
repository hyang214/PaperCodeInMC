...
public class TopKCSP {
	...
	public TopKCSP(int threadNumber, int minGap, int maxGap, int K, String posPath, String negPath, boolean a, boolean baseline) {
		this.minGap = minGap;
		this.maxGap = maxGap;
		this.K = K;
		try {
			...
			/** run this algorithm **/
			this.results = run(threadNumber, pos, neg, a, baseline);
	...
	private Results run(int threadNumber, ArrayList<String> pos, ArrayList<String> neg, boolean a, boolean baseline) {
...
//		HashMap<String, Element> negMap = GenerateGeneCandidateElement.INSTANCE.genCE(neg);
		...
		GenerateCandidatePattern gcp = new GenerateCandidatePattern(threadNumber, posMap, negMap, K, minGap, maxGap, pos.size(), neg.size(), baseline);
		gcp.genCP2();
